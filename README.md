...
Version 0.3 uses a hub-and-spoke structure: one compact coordinator for genuinely multi-phase work and five independent narrow Skills. A clear task can use one narrow Skill directly—or no Skill at all—without entering a mandatory chain.
...
| Measure | Baseline | Version 0.3 | Reduction |
|---|---:|---:|---:|
| Workflow Skills | 14 | 6 | 57.1% |
| Total `SKILL.md` words | 15,737 | 1,647 | 89.5% |
| Always-trigger router | Yes | No | Removed |

A normal narrow route loads one body of 233–281 words. The representative multi-phase route—core plus design and verification—is 871 words. The full 1,647-word package is not a mandatory prompt chain. Word count is a structural proxy, not a model-quality or tokenization guarantee.
...
The repository uses deterministic package checks, a nine-case routing specification, and isolated install/restore transaction smoke tests. The scenarios constrain intended behavior; they are not a live model benchmark:
